...
import './App.css';
import {connect, Provider, provider} from 'react-redux';
...
function Board({marks,player,gameOver,setGameOver,setMarks,setPlayer}){
  // const [marks,setMarks]= useState([0,0,0,0,0,0,0,0,0])//that those index where check marks
  ...
  const changeMark= (i)=>{
   ...
   if(m[i]===0 && !gameOver){
    ...
    alert('please refresh the block')
   }
  }
  return(
    <div className='Board'>
